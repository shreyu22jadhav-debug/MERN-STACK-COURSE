...
const Register = () => {
  ...
  const submitRegisterForm = async () => {
    try {
      // Send data to backend register API
      // Note: backend expects 'pass' not 'password'
      ...
      const apiResponse = await axios.post(
        `${import.meta.env.VITE_API_URL_BACKED}/register`,
        {
          name: name,
          email: email,
          pass: password   // backend expects 'pass' field
           // backend expects field name 'pass' not 'password'
        }
      )
